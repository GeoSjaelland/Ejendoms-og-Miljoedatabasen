SELECT
				Bosaettelse.FRAFLYT_KOMMUNENR AS Fraflytningskommunenr, QL60000V.TEKST AS Fraflytningskommune,
				Bosaettelse.KOMMUNENUMMER AS Bopælskommunenr,
				QL60000V_1.TEKST AS Bopælskommune, Bosaettelse.PERSONNUMMER,
				CASE
				...
				END AS CPR,
				Borger.FØDSEL_DATO AS Fødselsdato, CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, GETDATE()) / 365.25) AS Alder,
				CASE WHEN Bosaettelse.ADR_FRAFLYT_TS < '4713-12-31'
					THEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_FRAFLYT_TS, 10))) / 365.25)
				END AS Alder_ved_flytning,
