SELECT
				...
				CASE WHEN Bosaettelse.PERSONNUMMER > 1000000000
				...
				END AS CPR,
				CONVERT(varchar, Borger.FØDSEL_DATO, 23) AS Fødselsdato, CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, GETDATE()) / 365.25) AS Alder,
				...
				CASE WHEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10))) / 365.25) BETWEEN 0 AND 5
				  THEN '0 - 5 år'
				 WHEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10))) / 365.25) BETWEEN 6 AND 16
				  THEN '06 - 16 år'
				 WHEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10))) / 365.25) BETWEEN 17 AND 24
				  THEN '17 - 24 år'
				 WHEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10))) / 365.25) BETWEEN 25 AND 44
				  THEN '25 - 44 år'
				 WHEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10))) / 365.25) BETWEEN 45 AND 64
				  THEN '45 - 64 år'
				 WHEN CONVERT(INT, DATEDIFF(d, Borger.FØDSEL_DATO, CONVERT(datetime, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10))) / 365.25) > 64
				  THEN 'Over 64 år'
				END AS Aldersklasse,
				...
				CASE WHEN Borger.BORGERSTATUS_TS < '1753-01-01'
				 THEN CAST('1753-01-01' AS datetime)
				...
				CASE WHEN Borger.STATSB_RET_TS < '1753-01-01'
				 THEN CAST('1753-01-01' AS datetime)
				 ELSE CAST(LEFT(Borger.STATSB_RET_TS, 10) AS datetime)
				END AS Statsborgerret_ændring,
				CASE WHEN Borger.STATBORG_OPHØR_TS > '4713-12-31'
				 THEN CAST('4713-12-31' AS datetime)
				...
				CAST(Bosaettelse.KOMMUNENUMMER AS VARCHAR(3)) + CAST(Bosaettelse.VEJ_KODE AS VARCHAR(4)) + RTRIM(Bosaettelse.HUS_NUMMER) + LTRIM(Bosaettelse.ETAGE) + REPLACE(LTRIM(REPLACE(RTRIM(LTRIM(LTRIM(Bosaettelse.SIDE_DØRNR))), '0', ' ')), ' ', '0') AS UDVadresseID,
				Borger.PERSONNUMMER_MOR,
				Borger.PERSONNUMMER_FAR
FROM
				QL60000V RIGHT OUTER JOIN
				AA70100T AS Bosaettelse LEFT OUTER JOIN
				QL60000V AS QL60000V_1 ON Bosaettelse.KOMMUNENUMMER = QL60000V_1.KODE ON QL60000V.KODE = Bosaettelse.FRAFLYT_KOMMUNENR LEFT OUTER JOIN
				...
				LAND_ADRESSEPUNKT AS APU ON Bosaettelse.HUS_NUMMER = APU.HUS_NR AND Bosaettelse.VEJ_KODE = APU.VejKode AND Bosaettelse.KOMMUNENUMMER = APU.KommuneNummer
WHERE
				(YEAR(CONVERT(varchar, LEFT(Bosaettelse.ADR_TILFLYT_TS, 10), 23)) = 2019)
				AND (Bosaettelse.KOMMUNENUMMER = 336)
