...
FROM
				GS_BORGER_FLYTNING INNER JOIN
					(SELECT
						...
						CASE WHEN LEFT(GS_BORGER_FLYTNING_2.ADRESSE_ID, 5) = '33699'
						 THEN '336615004'
						 ELSE GS_BORGER_FLYTNING_2.Adresse_ID
						END AS ADRESSE_ID,
           ...
           WHERE
           	(YEAR(Fraflytningsdato) = 2020) AND (Bopælskommunenr = 336)
          ...
          AND GS_BORGER_FLYTNING_2.Fraflytningsdato = Maxdato.Fraflytningsdato) AS TMP_SIDSTEADRESSE
				...
				LEFT OUTER JOIN GS_DAR_ADRESSER
				ON TMP_SIDSTEADRESSE.ADRESSE_ID = GS_DAR_ADRESSER.ADRESSE_ID
WHERE
